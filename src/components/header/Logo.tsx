...
export function Logo({ size = 'default', animated = true, className = "" }: LogoProps) {
  // تكوين حجم الشعار
  const sizeClasses = {
    small: "px-10 py-2 text-xs",
    default: "px-14 py-3 text-sm xs:text-base",
    large: "px-16 py-4 text-base xs:text-lg"
  };
  
  // مسار نبضات القلب المستخدمة في الرسم البياني
  const heartbeatPath = "M0,10 L5,10 L7,2 L9,18 L11,0 L13,10 L15,5 L17,10 L20,10";
  
  return (
    <Link to="/" className={`relative mr-3 sm:mr-8 group ${className}`}>
      ...
      <div className={`relative flex items-center justify-center bg-black/5 dark:bg-white/5 backdrop-blur-sm rounded-[40px] border border-white/20 dark:border-gray-800/50 shadow-xl ${sizeClasses[size]}`}>
        <motion.div
        ...
        >
          ...
          <motion.div 
            className="absolute -z-10 inset-0 rounded-[40px] opacity-30"
            animate={{
              boxShadow: [
